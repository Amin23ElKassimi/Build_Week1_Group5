import socket
import subprocess  # <--- Necessario per eseguire il ping
import sys
...
def check_host_up(ip):
    ...
    Funziona su Linux/Kali.
    """
    try:
        # Eseguiamo il comando ping
        # -c 1: Invia solo 1 pacchetto
        # -W 1: Aspetta massimo 1 secondo per la risposta
        # stdout/stderr=DEVNULL serve a nascondere l'output del ping nel terminale
        ...
        # Se il codice di ritorno è 0, il ping ha avuto successo
        return output.returncode == 0
    except Exception:
        return False

def scan_port(target_ip, port, timeout=0.3): # Timeout basso per velocità
    try:
